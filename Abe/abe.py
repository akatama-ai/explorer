...
ABE_APPNAME = "Blockchain Explorer"
...
<body>
   ...
   <div class="container">
      <div class="navbar-header">
         ...
         <a class="navbar-brand" href="/">Chain Explorer</a>
      </div>
      <div class="navbar-collapse collapse" style="height: 1px;">
         <ul class="nav navbar-nav">
            ...
            <li><a href="/q">API</a></li>
         </ul>
         <p class="navbar-right navbar-text"><a href="#">
            <img src="%(dotdot)s%(STATIC_PATH)slogo32.png" alt="Follow us on twitter" title="Follow us on twitter" style="height:16px" />
         </a></p>
      </div>
   </div>
</div>
<div class="container">
    <div class="row">
      <h1 class="text-center" style="color:#2262AB"><b><a href="%(dotdot)s%(HOMEPAGE)s"></a> %(h1)s</b></h1>
   </div>
    
    %(body)s
    
</div>
      <script src="%(dotdot)s%(STATIC_PATH)sjquery.min.js"></script>
    <script src="%(dotdot)s%(STATIC_PATH)sbootstrap.min.js"></script>
</body>
</html>
"""
...
class Abe:
    ...
    def _show_block(abe, page, dotdotblock, chain, **kwargs):
        ...
        body += ['<p class="info-bl frist">Hash: ', b['hash'], '</p>\n']

    
        if b['hashPrev'] is not None:
            body += ['<p class="info-bl">Previous Block: <a href="', dotdotblock,
                     b['hashPrev'], '">', b['hashPrev'], '</a></p>\n']
       
      
        if b['next_block_hashes']:
            body += ['<p class="info-bl">Next Block: ']
        for hash in b['next_block_hashes']:
            body += ['<a href="', dotdotblock, hash, '">', hash, '</a></p>\n']
       
       
        body += [
            # ['<p class="info-bl">Height: ', b['height'], '</p>\n']
            # if b['height'] is not None else '',

            # '<p class="info-bl">Version: ', b['version'], '</p>\n',
            # '<p class="info-bl">Transaction Merkle Root: ', b['hashMerkleRoot'], '</p>\n',
            '<p class="info-bl">Time: ', format_time(b['nTime']), '</p>\n',
            '<p class="info-bl">Difficulty: ', format_difficulty(util.calculate_difficulty(b['nBits'])),
            ' (Bits: %x)' % (b['nBits'],), '</p>\n',

            ['<p class="info-bl">Cumulative Difficulty: ', format_difficulty(
                    util.work_to_difficulty(b['chain_work'])), '</p>\n']
            if b['chain_work'] is not None else '',

            # '<p class="info-bl">Nonce: ', b['nNonce'], '</p>\n',
            '<p class="info-bl">Transactions: ', len(b['transactions']), '</p>\n',
            '<p class="info-bl">Value out: ', format_satoshis(b['value_out'], chain), ' ',  escape(chain.code3), '</p>\n']
            # '<p class="info-bl">Transaction Fees: ', format_satoshis(b['fees'], chain), '</p>\n',

            # ['<p class="info-bl">Average Coin Age: %6g' % (b['satoshi_seconds'] / 86400.0 / b['chain_satoshis'],),
            #  ' days</p>\n']
            # if b['chain_satoshis'] and (b['satoshi_seconds'] is not None) else '',

            #'' if b['satoshis_destroyed'] is None else
            # ['<p class="info-bl">Coin-days Destroyed: ',
            #  format_satoshis(b['satoshis_destroyed'] / 86400.0, chain), '</p>\n'],

            # ['<p class="info-bl">Cumulative Coin-days Destroyed: %6g%%<br />\n' %
        ...
        for tx in b['transactions']:
            ...
            for txout in tx['out']:
                ...
                body += [abe.format_addresses(txout, page['dotdot'], chain), ': ',
                         format_satoshis(txout['value'], chain), ' ',  escape(chain.code3), '<br />']
    ...
    def show_tx(abe, page, tx):
        ...
        def row_to_html(row, this_ch, other_ch, no_link_text):
            ...
            body += [
                '</td>\n',
                '<td>', format_satoshis(row['value'], chain), ' ', escape(chain.code3), '</td>\n',
